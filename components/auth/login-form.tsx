...
export function LoginForm() {
  ...
  const handleContinue = async (e: React.FormEvent) => {
    ...
    try {
      // Verificar si el usuario existe usando la API de Supabase
      const { data, error } = await supabase.auth.admin.listUsers();

      if (error) {
        // Si no tenemos permisos de admin, intentamos un enfoque diferente
        // Intentamos hacer un sign in con una contraseña vacía para ver si el usuario existe
        const { error: signInError } = await supabase.auth.signInWithPassword({
          email,
          password: "temporary_password_for_validation",
        });

        // Si el error es específico de contraseña incorrecta, significa que el usuario existe
        if (
          signInError &&
        ...
        // Si tenemos permisos de admin, verificamos directamente
  ...
  return (
    <div className="w-full p-0">
      <div className="mb-4 text-center">
        ...
        <p className="text-muted-foreground text-lg">Log in to your account</p>
      </div>
      <div>
